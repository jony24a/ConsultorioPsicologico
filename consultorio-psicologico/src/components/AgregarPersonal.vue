<template>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 md:p-10 max-w-3xl mx-auto">
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-6">
              ...
              <input v-model="personal.tipo_documento" id="tipo_documento" required
            ...
            <div class="mb-6">
              ...
              <input v-model="personal.genero" id="genero" required
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" />
            </div>
            <!-- Estrato -->
            <div class="mb-6">
              <label for="estrato" class="block mb-2 text-sm font-medium text-gray-900">Estrato:</label>
              <input v-model="personal.estrato" id="estrato" required
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" />
            </div>
            <!-- Barrio -->
            <div class="mb-6">
  ...
  import { Personal } from '../types';  // Suponiendo que tienes el tipo Personal
  
  // Suponiendo que tienes un servicio que maneja la creación de personal
  import { agregarPersonal } from '../services/personalService';
  
  export default defineComponent({
    name: 'AgregarPersonal',
    setup() {
      // Definir el modelo de datos para el personal
      const personal = ref<Personal>({
        ...
        turno: '', // Este campo es obligatorio siempre
      });
  
      // Función para manejar el envío del formulario
      const submitForm = async () => {
        try {
          // Validación para asegurarnos de que el campo "turno" esté presente
          if (!personal.value.turno) {
            alert('El turno es obligatorio.');
            return;
          }
  
          // Llamar al servicio para agregar el personal
  ...
  <style scoped>
  /* Aquí puedes añadir tu estilo si es necesario */
  </style>
